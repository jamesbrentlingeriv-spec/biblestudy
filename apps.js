...
class BibleStudyApp {
    ...
    async loadChapter() {
        ...
        content.innerHTML = '<div class="loading-shimmer w-3/4"></div><div class="loading-shimmer w-full"></div><div class="loading-shimmer w-5/6"></div>';

        const data = await bibleData.fetchChapter(this.currentBook, this.currentChapter, this.currentTranslation);

        if (!data) {
            content.innerHTML = '<div class="text-center text-red-500 p-4">Failed to load chapter. Please try again.</div>';
            return;
        }
    ...
    renderChapter(data) {
        ...
        if (!data.verses || data.verses.length === 0) {
            content.innerHTML = '<div class="text-center text-slate-500">No verses found</div>';
            return;
        }
    ...
    formatVerseText(verse) {
        ...
        return safeText.replace(/"([^"]+)"/g, '<span class="words-of-christ">"$1"</span>');
    }
    ...
    navigateChapter(direction) {
        const currentBookIndex = bibleData.books.findIndex((b) => b.name === this.currentBook);
    ...
    toggleMobileMenu() {
        ...
        const isOpen = biblePanel.classList.contains('panel-open-left') || sidebar.classList.contains('panel-open-right');
        if (isOpen) {
